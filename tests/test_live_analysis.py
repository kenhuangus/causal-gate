...
from causalgate.demo import FIXTURE_HASH, run_demo
from causalgate.live_analysis import AnalysisUnavailable, _calls, analyze_live, generate_recorded_artifact, minimized_trace, verify_recorded_artifact
...
def test_ephemeral_byok_key_can_power_one_request_without_environment_storage(monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    run = run_demo("baseline")
    artifact = analyze_live(run, client=Client(valid_output(run)), api_key="synthetic-ephemeral-key-at-least-32-bytes")
    assert artifact.mode == "live"
    assert os.getenv("OPENAI_API_KEY") is None


def test_invalid_byok_key_fails_before_provider_call(monkeypatch):
    monkeypatch.delenv("OPENAI_API_KEY", raising=False)
    client = Client(valid_output(run_demo("baseline")))
    with pytest.raises(AnalysisUnavailable, match="supplied API key is invalid"):
        analyze_live(run_demo("baseline"), client=client, api_key="short key")
...
def test_disabled_gate_never_invokes_client(monkeypatch):
    monkeypatch.setenv("CAUSALGATE_LIVE_ANALYSIS_ENABLED", "false"); client = Client(valid_output(run_demo("baseline")))
...
def test_recorded_verifier_fails_missing_unless_explicitly_optional(tmp_path: Path):
    ...
    env = {**os.environ, "CAUSALGATE_RECORDED_ANALYSIS_OPTIONAL": "false"}
    assert subprocess.run(command, env=env, capture_output=True, text=True).returncode == 1
    env["CAUSALGATE_RECORDED_ANALYSIS_OPTIONAL"] = "true"
